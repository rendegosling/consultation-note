...
async function migrateToLatest() {
  ...
  const migrator = new Migrator({
    ...
    provider: {
      async getMigrations() {
        ...
        logger.debug(COMPONENT_NAME, 'Loading migrations from directory', {
          path: migrationsPath,
        });

        const files = await fs.readdir(migrationsPath);
        logger.debug(COMPONENT_NAME, 'Found migration files', {
          count: files.length,
          files: files.join(', '),
        });
  ...
  if (results?.length) {
    logger.info(COMPONENT_NAME, 'Migrations completed successfully', {
      ...
      migrations: results.map((it) => it.migrationName).join(', '),
    });
  } else {
